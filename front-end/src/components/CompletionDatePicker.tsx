...
export default function CompletionDatePicker({ date, onChange }: CompletionDatePickerProps) {
  ...
  function incDec(field: "m" | "d" | "y", dir: 1 | -1, e: React.KeyboardEvent<HTMLInputElement>) {
    e.preventDefault();
    if (field === "m") {
      const n = +(mm || "1"); let next = Math.min(12, Math.max(1, n + dir)); const out = pad2(next);
      setMM(out);
      ...
      const n = +(dd || "1"); let next = Math.min(maxD, Math.max(1, n + dir)); const out = pad2(next);
      setDD(out); commitIfComplete(yyyy, mm, out);
    } else {
      const n = +(yyyy || "2000"); let next = Math.max(1, n + dir); const out = String(next).slice(0, 4);
